  ...
  rwData.forEach(rw => {
    ...
    card.innerHTML = `
      <div class="img-container">
        <img src="${rw.gambar[0]}" alt="${rw.nama}" class="card-img">
      </div>
...
const modalText = document.getElementById('rw-modal-text');
const rwMainImg = document.getElementById('rw-main-img');
const rwThumbnails = document.getElementById('rw-thumbnails');
const rwSlideLeft = document.getElementById('rw-slide-left');
const rwSlideRight = document.getElementById('rw-slide-right');
const closeBtn = document.querySelector('.close-btn');

let currentRwImages = [];
let currentRwImageIndex = 0;

function updateRwMainImage(index) {
  if (index < 0) index = currentRwImages.length - 1;
  if (index >= currentRwImages.length) index = 0;
  
  currentRwImageIndex = index;
  
  rwMainImg.style.opacity = 0;
  setTimeout(() => {
    rwMainImg.src = currentRwImages[currentRwImageIndex];
    rwMainImg.style.opacity = 1;
  }, 150);
  
  document.querySelectorAll('#rw-thumbnails img').forEach((t, i) => {
    if (i === currentRwImageIndex) {
...
function openModal(rw) {
  ...
  currentRwImages = rw.gambar;
  currentRwImageIndex = 0;
  rwMainImg.src = currentRwImages[0];
  
  rwThumbnails.innerHTML = '';
  currentRwImages.forEach((imgSrc, index) => {
    const thumb = document.createElement('img');
    thumb.src = imgSrc;
    if (index === 0) thumb.classList.add('active');
    
    thumb.addEventListener('click', () => {
      updateRwMainImage(index);
    });
...
if (rwSlideLeft) {
  rwSlideLeft.addEventListener('click', () => updateRwMainImage(currentRwImageIndex - 1));
}

if (rwSlideRight) {
  rwSlideRight.addEventListener('click', () => updateRwMainImage(currentRwImageIndex + 1));
}
